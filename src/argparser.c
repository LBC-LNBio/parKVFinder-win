	...
	*whole_protein_mode = 1;
	...
	while (1) {
		...
		switch (c) {
			...
			case 'L':
//				snprintf (LIGAND_NAME, 500, "%s", realpath (optarg, NULL)); /* Windows */
				_fullpath(LIGAND_NAME, optarg, 500); /* Windows */
				*ligand_mode = 1;
				l_flag = 1;
            ...
            /* DICTIONARY */
			case 'd':
//				snprintf (dictionary_name, 500, "%s", realpath (optarg, NULL)); /* Windows */
...
//            snprintf (PDB_NAME, 500, "%s", realpath (argv[optind], NULL)); /* Windows */
            _fullpath(PDB_NAME, argv[optind], 500); /* Windows */

            /* Loop through string PDB_NAME */
            for (i = 0; PDB_NAME[i] != '\0'; i++) {
                /* Get position of last bar symbol in PDB_NAME */
                if (PDB_NAME[i] == '\\') /* Windows */
                    bar = i;
	...
	if (!d_flag){
		...
		strcat (dictionary_name, "\\dictionary"); /* Windows */
		if (verbose_flag)
